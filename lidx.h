...
//  struct lidx, lidx : structure premettant de stocker différentes 
//    informations, entre autre, une hashtable contenant les mots du lexique en 
//    tant que key et une struct ht_val en value, un holdall filenames contenant
...
//    pas dans un fichier, enfin, il y a aussi une struct settings qui ??????????????????????????????????????????? 
...
//    succès, NULL en cas d'échec. Settings sera def ???????????????????????????????
